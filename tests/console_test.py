...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Safi"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Safi'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="sherpa" age=27 height=5.5')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'sherpa'", cout.getvalue().strip())
            self.assertIn("'age': 27", cout.getvalue().strip())
            self.assertIn("'height': 5.5", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="sherpa5@gmail.com" password="012"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('sherpa5@gmail.com', result)
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="sherpa5@gmail.com", password="012")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('sherpa5@gmail.com', result)
            self.assertIn('012', result)
            self.assertIn('sherpa25@gmail.com', cout.getvalue())
            self.assertIn('012', cout.getvalue())
            cursor.close()
            dbc.close()
    ...
    def test_db_count(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            cons.onecmd('create State name="Casa"')
            clear_stream(cout)
